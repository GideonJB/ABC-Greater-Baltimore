...
const Root = ({ state, actions }) => {
  ...
  const managementIDS = [
    213, 1303, 1299, 1297, 1301, 1295, 1330, 1544, 1546, 1635, 1633, 1656, 1668,
    1640, 2156, 1100, 206, 207, 217, 1255, 1393, 1635, 1661, 2316, 2133, 2791,
  ];
  ...
  useEffect(() => {
    console.log("EFFECT");
    console.log("DATA", state.source.get(state.router.link));
  ...
  return (
    <Parent id="print-parent" style={{ visibility: state.theme.isVisible }}>
      ...
      {state.source.get(state.router.link).isHome ? null : (
        <Header style="inner" />
      )}
          ...
          <AnimationWrapper className="">
            {transitions((props, item) => {
              ...
              return (
                <animated.div
                  id="scrollBody"
                  className={state.router.link !== "/" ? "" : "homepage"}
                  style={props}
                >
                  <SpacingDiv
                    className={state.router.link !== "/" ? "" : "homepage"}
                  >
                    <Switch location={item}>
                      ...
                      <CaresBlog
                        when={state.router.link.includes("cares-blog")}
                      />
                      <Blog when={state.router.link.includes("blog")} />
                      <EventsPage
                        when={
                          dataitem.isPage && state.router.link === "/events/"
                        }
                      />
                      <PdpPage
                        when={dataitem.isPage && state.router.link === "/pdp/"}
                      />
                      <SearchPage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/find-a-contractor/"
                        }
                      />
                      <InvoicePage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/pay-invoice/"
                        }
                      />
                      <AboutUsPage
                        when={
                          dataitem.isPage && state.router.link === "/about-us/"
                        }
                      />
                      <LogicScholarshipPage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/logic-scholarship/"
                        }
                      />
                      <ApprenticeAppPage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/apprenticeship-application/"
                        }
                      />
                      <JoinABCPage
                        when={
                      ...
                      <ApprenticeResumePage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/apprentice-resume/"
                        }
                      />
                      ...
                      <BestSponsorPage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/pro-sponsorship/"
                        }
                      />
                      <TaskTrainingPage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/task-training/"
                        }
                      />
                      <FacilityRentalPage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/facility-rental/"
                        }
                      />
                      <CompanyUpdatePage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/company-update/"
                        }
                      />
                      <SchoolDataPage
                        when={
                          dataitem.isPage &&
                          state.router.link === "/school-data/"
                        }
                      />
                      <RegistrationPage
                        when={state.router.link === "/register/"}
                      />
                      <ResumePrint
                        when={state.router.link === "/resume-print/"}
                      />
                      <ABCCaresPage
                        when={state.router.link === "/abc-cares/"}
                      />
            ...
            <Switch>
              <PageBackground
                when={data.isPage && membershipIDS.includes(data.id)}
                membership
              />
              <PageBackground
                when={data.isPage && apprenticeshipIDS.includes(data.id)}
                apprenticeship
              />
              <PageBackground
                when={data.isPage && eventsIDS.includes(data.id)}
                events
              />
              <PageBackground
                when={data.isPage && safetyIDS.includes(data.id)}
                safety
              />
              <PageBackground
                when={data.isPage && managementIDS.includes(data.id)}
                management
              />
              <PageBackground
                when={data.isPage && politicalIDS.includes(data.id)}
                political
              />
              <PageBackground
                when={data.isPage && membersIDS.includes(data.id)}
                members
              />
            </Switch>
          </AnimationWrapper>
