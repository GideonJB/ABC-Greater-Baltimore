...
const AboutUsPage = ({state, actions}) => {
  ...
  const staffListSplit = staffList.default.map((e, i) => {
    return i % columnNumber === 0 ? staffList.default.slice(i, i + columnNumber) : null;
  }).filter(e => { return e; });
  const boardListSplit = boardList.default.map((e, i) => {
    return i % columnNumber === 0 ? boardList.default.slice(i, i + columnNumber) : null;
  }).filter(e => { return e; });
          ...
          return(
            <Row>
              {arrays.map((data, index) => {
                ...
                return(
                  <a href={`mailto:${data.email}`} target="_blank">
                  <Column imageBackground={Image.default}>
                    <Overlay>
                      <Name>{data.Name}</Name>
                      <Title>{data.Title}</Title>
                      {/* <Description>{data.Description}</Description> */}
